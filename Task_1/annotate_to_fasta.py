...
Usage:
    python3 annotate_to_fasta.py filtered_input.csv output.fasta
    (where filtered_input.csv is the CSV file generated by clean_msdata.py
    and output.fasta is the desired name of the FASTA file generated.)
"""
...
def parse_master_mods(mods_string: str, expected_accession: str) -> dict:
    ...
    result = {}
    for block_match in pattern.finditer(rest):
        count_str, inside = block_match.groups()
        total_count = int(count_str)

        full_block = block_match.group(0)
        if re.search(r"xMethyl", full_block, re.IGNORECASE):
            tag = "Me1"
        elif re.search(r"xDimethyl", full_block, re.IGNORECASE):
            tag = "Me2"
        else:
            tag = "Me3"
        ...
        positions = [int(m.group(1)) for m in pos_pattern.finditer(inside)]

        # Distribute counts
        if len(positions) == 1:
            # If one position is present assign it full count
            result[positions[0]] = (tag, total_count)
        else:
            for pos in positions:
                # If multiple positions give each count = 1
                result[pos] = (tag, 1)
...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        count_inside = sum(1 for p in mods_dict if pep_start <= p <= pep_end)
        header = (
            f"{accession} "
